...
export default function AuthSection({
  ...
  const accessAlbum = async () => {
    try {
      ...
      localStorage.setItem(res.data.access_type === "master" ? "master_key" : "readonly_key", accessKey);
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4 items-center">
        <button onClick={() => setView("create")} className="px-4 py-2 animate-heartbeat text-2xl font-pixelcute bg-blue-500 text-white rounded-lg shadow hover:bg-blue-600">
          Create a new album
        </button>
        <button onClick={() => setView("access")} className="px-4 py-2 animate-heartbeat font-pixelcute text-2xl bg-green-500 text-white rounded-lg shadow hover:bg-green-600">
          See your album!
        </button>

        {view === "create" && (
      <div className="mt-4 flex flex-col items-center gap-4">
        {/* Input para el nombre del álbum */}
        <input
